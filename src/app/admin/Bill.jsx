...
const IndexPage = () => {
  ...
  const [products, setProducts] = useState([{ id: 1, name: '', category: '', quantity: 0, price: 0 }]);
  ...
  <div style={{ marginBottom: '20px' }}>
    ...
    <div style={{ display: 'flex', marginBottom: '10px' }}>
      <input style={{ ...inputStyle, marginRight: '10px', flex: '1' }} type="text" value={contact} onChange={(e) => setContact(e.target.value)} placeholder="Contact" />
      <input style={{ ...inputStyle, flex: '1' }} type="text" value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)} placeholder="Payment Method" />
    </div>

    <div>
      <h3>Product Details</h3>
      {products.map((product, index) => (
        <div key={index} style={{ ...listItemStyle, display: 'flex', justifyContent: 'space-between' }}>
          <div style={{ flex: '1', marginRight: '10px' }}>
            <input style={inputStyle} type="text" value={product.name} onChange={(e) => updateProduct(index, 'name', e.target.value)} placeholder="Product Name" />
          </div>
      ...
      <button style={addProductButtonStyle} onClick={addProduct}>Add Product</button>
    </div>

    <input style={inputStyle} type="text" value={total} placeholder="Total" readOnly />
  ...
  <h2 style={{ textAlign: 'center', color: '#333' }}>Bills</h2>
    ...
    <button style={{ ...buttonStyle, display: 'block', margin: '0 auto' }} onClick={fetchBills}>Refresh Bills</button>
  </div>
</div>
);
};
